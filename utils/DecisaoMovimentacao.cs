namespace CartolaFCRPG.utils {
    ...
    public class DecisorDeMovimentacao
    {
        ...
        {
            var mapa = TaticasMovimentacao.Mapas[jogador.Perfil];
            var pesos = timeTemPosse ? mapa.PesosComBola : mapa.PesosSemBola;

            var alternativas = new List<(PosicaoCampo pos, int peso)>();

            for (int i = 0; i < ALTURA_CAMPO; i++)
            {
                for (int j = 0; j < LARGURA_CAMPO; j++)
                {
                    int peso = pesos[i, j];
                    if (peso > 0)
                        alternativas.Add((new PosicaoCampo(i, j), peso));
                }
            }

            if (!alternativas.Any()) return null;
